        ...
        if recursive and os.path.isdir(file):
            for dirpath, dnames, fnames in os.walk(file):
                ...
                for pattern in exclude:
                    ...
                    # Updated to remove './' when running on the root of the repo
                    dnames[:] = [
                        x for x in dnames
                        if
                        not fnmatch.fnmatch(os.path.join(dirpath, x).lstrip('./'), pattern)
                    ]
                    fpaths = [
                        x for x in fpaths if not fnmatch.fnmatch(x.lstrip('./'), pattern)
                    ]
                for f in fpaths:
